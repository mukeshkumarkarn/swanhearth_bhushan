...
<div class="main-innerpage w-100 float-start">
    <div class="container">
        <div class="innerpages">
            ...
            <span><a href="{{url('/')}}"> Home </a> &gt; <span class="font-color-pink">Email Request Received</span></span>
        </div>
    </div>
</div>
                        ...
                        <div class="pannal-box mt-0">
                            ...
                            <div class="pannal-content">
                                <ul class="nav nav-tabs profile-setting">
                                    ...
                                    <li class="nav-item"><a class="nav-link {{ $requestEmailsCount > 0 ? 'note' : '' }} active show" data-toggle="tab" href="#tab-16"><span><i class="fa-solid fa-check-double"></i></span> Email Request Received<div class="note-count2 EmailNot">{{$requestEmailsCount}}</div></a></li>
                ...
                <div class="col-lg-8 col-md-12 col-12">
                    <div class="my-profile tab-content">
                        <div class="tab-pane active show" id="tab-16">
                            <div class="heading">
                                <h2><strong>Email Request</strong> Received</h2>
                                <div class="heart-line"> <i class="fas fa-heart"></i> </div>
                            </div>
                            <div class="gallery4-wrapper">
                                <div class="row">
                                    @if(count($requestEmails) > 0)
                                    @foreach($requestEmails as $requestEmail)
                                    <div class="col-lg-6 col-md-6 col-xs-12 col-sm-12">
                                        <h6 class="messageShow" style="color:green;"></h6>
                                        <h6 class="errorshow error"></h6>
                                        <div class="tab_image_wrapper">
                                            <div class="tab_image">
                                                <figure>
                                                    @if($requestEmail->profile_img)
                                                    <img src="{{ url('assets/images/profile_img'.'/'. $requestEmail->profile_img) }}" class="img-responsive" alt="profile-img">
                                                    @else
                                            ...
                                            <div class="tab_image_text">
                                                <div class="project_title">
                                                    <h4><a href="javascript:;">{{$requestEmail->name}}</a></h4>
                                                    <p> {{$requestEmail->city}}, {{$requestEmail->state}} </p>
                                                    <!-- <p> 85% Match </p> -->
                                                </div>
                                                <div class="req-btn-profile">
                                                    <ul class="media-list">
                                                        <li data-tooltip="Send Message"><a href="{{url('message').'/'.$requestEmail->user_ref}}"><i><img src="{{asset('assets/images/icon/send-message.png')}}" alt="send-message" /></i></a></li>
                                                        @if($requestEmail->request_email_status===1)
                                                        <button class="main-btn1 accept-request-email" data-reference="{{$requestEmail->id}}" id="{{$requestEmail->id}}">Accepted</button>
                                                        <button class="main-btn1 deny-request-email" data-reference="{{$requestEmail->id}}" id="{{$requestEmail->id}}">Deny</button>
                                                        @endif
                                                        @if($requestEmail->request_email_status===2)
                                                        <p>{{$requestEmail->email}}</p>
                                                        @endif
                                                    </ul>
                                                </div>
                                            </div>
                            ...
                            <div class="d-flex justify-content-center">
                                {{ $requestEmails->links('pagination::bootstrap-4') }}
                            </div>
                        </div>
                    </div>
                </div>
...
<script>
    $(document).ready(function() {
        var id = $('.user_id').val();
        var url = '/api/email-request-notification-remove/' + id;
        ...
        $.ajax({
            ...
            success: function(data) {
                $('.EmailNot').text(data).show();
                $('.EmailNot').css('background-color', 'white').show();
            }
        });
    });
</script>